...
const getLists = async (config: TrelloConfig): Promise<TrelloListEntry[]> => {
  const res = await FetchQueue.getInstance().fetch(`https://api.trello.com/1/boards/${config.boardId}/lists?key=${config.key}&token=${config.token}`);
...
export const getCards = async (config: TrelloConfig, listEntry: TrelloListEntry): Promise<TrelloCard[]>=> {
  const res = await FetchQueue.getInstance().fetch(`https://api.trello.com/1/lists/${listEntry.id}/cards?key=${config.key}&token=${config.token}`);
